...
# This migration comes from spotlight (originally 20210308090000)
...
class MigrateCaptionValuesForTitleKey < ActiveRecord::Migration[5.2]
  def up
    ...
    change_caption_field_of(Spotlight::Page, from: CatalogController.blacklight_config.index.title_field,
                                             to: Spotlight::PageConfigurations::DOCUMENT_TITLE_KEY)
  end

  def down
    Spotlight::Page.reset_column_information
    change_caption_field_of(Spotlight::Page, from: Spotlight::PageConfigurations::DOCUMENT_TITLE_KEY,
                                             to: CatalogController.blacklight_config.index.title_field)
  end
  ...
  def change_caption_field_of(scope, from:, to:)
    scope.find_each do |page|
      ...
      page.content.select do |block|
        block['primary-caption-field'] == from || block['secondary-caption-field'] == from
      end.each do |block|
        changed = true
